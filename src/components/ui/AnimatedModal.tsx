...
import DefaultSVG from "../../../public/assets/ModalsBG/undraw_add_document_re_mbjx.svg";
import React, {
  ReactNode,
  createContext,
  useContext,
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className="fixed [perspective:800px] [transform-style:preserve-3d] inset-0 h-full w-full flex items-center justify-center z-[1005]"
        >
          <Overlay />
          ...
          <motion.div
            ...
            className={cn(
              "bg-white dark:bg-black border border-transparent md:rounded-[32px] relative z-[1010] flex flex-col overflow-hidden shadow-md shadow-white",
              className
            )}
            initial={{
              opacity: 0,
...
const Overlay = ({ className }: { className?: string }) => {
  return (
    <motion.div
      ...
      className={`fixed inset-0 h-full w-full bg-black bg-opacity-50 z-[1000] ${className}`}
    ></motion.div>
  );
};
